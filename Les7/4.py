...
class TownCar(Car):
    ...
    def show_speed(self):
        if self.speed > 60:
            print(f'Осторожно! Превышение скорости {self.speed} км/ч')
        else:
            print(f'Скорость {self.speed} км/ч')
...
viper = SportCar('желтый', 'Dodge Viper')
test_drive(viper)

voxy = WorkCar('серый', 'Toyota Voxy')
test_drive(voxy)

impala = PoliceCar('белый', 'Chevrolet Impala')
test_drive(impala)